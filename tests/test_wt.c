...
static void
test_reject_status_validation (void)
{
    int accepted;

    accepted = -1;
    assert(0 == lsquic_wt_test_reject_status_validation(0, &accepted));
    assert(accepted == 1);

    accepted = -1;
    assert(0 == lsquic_wt_test_reject_status_validation(199, &accepted));
    assert(accepted == 1);

    accepted = -1;
    assert(0 == lsquic_wt_test_reject_status_validation(200, &accepted));
    assert(accepted == 0);

    accepted = -1;
    assert(0 == lsquic_wt_test_reject_status_validation(299, &accepted));
    assert(accepted == 0);

    accepted = -1;
    assert(0 == lsquic_wt_test_reject_status_validation(300, &accepted));
    assert(accepted == 1);
}
...
static void
test_dgq_overflow_rejected (void)
{
    int overflow_rejected;

    overflow_rejected = 0;
    assert(0 == lsquic_wt_test_dgq_overflow_rejected(0, &overflow_rejected));
    assert(overflow_rejected);

    overflow_rejected = 0;
    assert(0 == lsquic_wt_test_dgq_overflow_rejected(1, &overflow_rejected));
    assert(overflow_rejected);
}
